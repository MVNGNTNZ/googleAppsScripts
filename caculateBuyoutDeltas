function calculateBuyoutDeltas() {
    ...
    var jobsSB = scoreboardJobs.getRange(2, 2, lastRowSB - 1, 1).getValues().flat().map(job => job.toLowerCase());
    ...
    var jobsBB = buyoutBaseline.getRange(2, 2, lastRowBB - 1, 1).getValues().flat().map(job => job.toLowerCase());

    // Get the sums from current and previous report in Buyout Baseline
    var currentSums = buyoutBaseline.getRange(2, currentReport, lastRowBB - 1, 1).getValues().flat();
    var previousSums = buyoutBaseline.getRange(2, previousReport, lastRowBB - 1, 1).getValues().flat();
    ...
    scoreboardJobs.getRange(1, 19, scoreboardJobs.getMaxRows(), 1).setBorder(false, true, false, true, false, false);

    var outputRange = scoreboardJobs.getRange(2, 19, deltas.length, 1);
    outputRange.setValues(deltas);
    outputRange.setNumberFormat("_($* #,##0.00_);_($* (#,##0.00);_($* \"-\"??_);_(@_)");
    ...
    var data = scoreboardJobs.getRange(2, 1, lastRowSB - 1, 1).getValues().flat();
    var startRow = 2;
    for (var i = 0; i < data.length; i++) {
        if (data[i] === "Totals") {
            var endRow = i + 1; // Row index in the sheet (1-based)
            var sumRange = scoreboardJobs.getRange(startRow, 19, endRow - startRow, 1);
